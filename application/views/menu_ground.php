...
<body class="g-sidenav-show  bg-gray-200">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Ground</li>
          </ol>
          <h6 class="font-weight-bolder mb-0">Monitoring Ground</h6>
        </nav>
              ...
              <div class="card-body">
                <div class="row">
                  ...
                  <div class="col-4 col-md-auto d-flex flex-column justify-content-center align-items-center">
                    <div class="connection-indicator gnd-in-indicator mb-2" id="connectionStatus"></div>
                    <div class="device-status gnd-in-status" id="deviceStatusText">DEVICE OFFLINE</div>
                  </div>
                </div>
              </div>
                <div class="row">
                  <div class="col-lg-6 col-md-6">
                    <div class="card-body">
                      <div id="v_gnd1b" style="height: 250px; min-width: 225px; max-width: 290px; margin: 0 auto;"></div>
                    </div>
                  </div>
                  <div class="col-lg-6 col-md-6">
                    <div class="card-body">
                      <div id="v_gnd1a" style="height: 250px;"></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="col-lg-12 col-md-12 mb-4">
              ...
              <div class="card-body">
                <div class="row">
                  ...
                  <div class="col-4 col-md-auto d-flex flex-column justify-content-center align-items-center">
                    <div class="connection-indicator gnd-out-indicator mb-2" id="connectionStatus"></div>
                    <div class="device-status gnd-out-status" id="deviceStatusText">DEVICE OFFLINE</div>
                  </div>
                </div>
              </div>
                <div class="row">
                  <div class="col-lg-6 col-md-6">
                    <div class="card-body">
                      <div id="v_gnd2b" style="height: 250px; min-width: 225px; max-width: 290px; margin: 0 auto;"></div>
                    </div>
                  </div>
                  <div class="col-lg-6 col-md-6">
                    <div class="card-body">
                      <div id="v_gnd2a" style="height: 250px;"></div>
                    </div>
                  </div>
                </div>
              </div>
  ...
  <script type="text/javascript">
        ...
        events: {
          load: function () {
            ...
            setInterval(function () {
              ...
              series.addPoint([x, vgnd1], true, true);
            },2000);
          }
        }
      },
      ...
      chart: {
        ...
        events: {
          load: function () {
            ...
            setInterval(function () {
              var x = (new Date()).getTime();
              series.addPoint([x, vgnd2], true, true);
            },2000);
          }
        }
      },
    ...
    $(document).ready(function() {
      setInterval(function () {
        $.ajax({
          url:  '<?php echo base_url(); ?>getdata/ground',
          success: function (point) {
            ...
            vgnd1 = data1;
            vgnd2 = data2;
            var point1 = $('#v_gnd1b').highcharts().series[0].points[0];       
            point1.update(vgnd1);
            var point2 = $('#v_gnd2b').highcharts().series[0].points[0];       
            point2.update(vgnd2);
...
<script>
    ...
    setInterval(function() {
        $.ajax({
            url: '<?php echo base_url(); ?>writedata/check_last_data_gnd_in',
            success: function(response){
                if(response == "OFFLINE") {
                    $('.gnd-in-indicator').css('background-color', 'red');
                    $('.gnd-in-status').css('border-color', 'red').text('DEVICE OFFLINE');
                } else if(response == "ONLINE") {
                    $('.gnd-in-indicator').css('background-color', 'green');
                    $('.gnd-in-status').css('border-color', 'green').text('DEVICE ONLINE');
                } else if(response == "TRANSMIT") {
                    $('.gnd-in-indicator').css('background-color', 'black');
                    $('.gnd-in-status').css('border-color', 'black').text('TRANSMIT DATA');
                }
            } 
        });

        $.ajax({
            url: '<?php echo base_url(); ?>writedata/check_last_data_gnd_out',
            success: function(response){
                if(response == "OFFLINE") {
                    $('.gnd-out-indicator').css('background-color', 'red');
                    $('.gnd-out-status').css('border-color', 'red').text('DEVICE OFFLINE');
                } else if(response == "ONLINE") {
                    $('.gnd-out-indicator').css('background-color', 'green');
                    $('.gnd-out-status').css('border-color', 'green').text('DEVICE ONLINE');
                } else if(response == "TRANSMIT") {
                    $('.gnd-out-indicator').css('background-color', 'black');
                    $('.gnd-out-status').css('border-color', 'black').text('TRANSMIT DATA');
                }
            } 
        });
      }, 1500); 
